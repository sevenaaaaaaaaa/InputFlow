...
| 🐾 **有生命的桌宠** | emoji 兜底 + VRM 3D 渲染器（注视/眨眼/物理），支持导入自己的二次元形象 |
...
桌宠 = **皮肤资产 + 渲染器**（[契约见 ADR-0007](docs/adr/0007-desktop-pet-renderers.md)），分三层：

| 层级 | 说明 |
|---|---|
| **emoji 兜底** | 内置 10 个系统表情形象（🐱🐶🦊🐼🐹🐰🐧🤖👧🧑‍🎨），零下载、放大也清晰 |
| **VRM 3D 渲染器** | `WKWebView + three.js + three-vrm`，本地离线运行：透明渲染、**跟随鼠标注视**（头部+眼球）、随机眨眼、呼吸与打字/上屏动效、VRM 弹簧骨物理；全身/半身画幅与缩放可调 |
| **导入 VRM** | 菜单「桌宠形象 → 导入 VRM 模型…」，用 **VRoid Studio（免费）** 捏好角色导出 `.vrm` 即可，动效自动继承 |

行为：输入焦点在哪块屏幕，桌宠就在哪块屏幕互动（跨屏自动跟随）；点击 = 中/英切换，
悬停浮出快捷圆钮（模式 / 统计），右键菜单（标点 / 昨日输入小结 / 权限与隐私），
拖动位置记忆。气泡只报本机任务，**永不承载付款与营销**
（[边界见 ADR](docs/adr/0006-notifications-and-stats.md)）。

> 示例形象包（`examples/plugins/pet-*`）用于演示插件契约；想要真正好看的二次元形象，
> 建议用 VRoid Studio 自制或从 VRoid Hub 挑一个 CC0/CC-BY 角色后「导入 VRM」。
...
| L1 端上语音 | 系统本地语音识别：**边说边落字**（实时预编辑）+ 语音候选融合，final **标点二次确认**（原样/。/，/！/？），打字自动接管；不支持端上则禁用，**绝不回退云端** | 关闭 |
| L2 小模型 | Gemma 3 / Qwen / Whisper 一键下载（sha256 校验，本机运行） | 关闭 |
| L2 同声传译 | 菜单「同声传译 → 译成英文/日文」武装后，语音边说边出**译文候选**（译文/原文按 1/2 或点击切换上屏）；尾段流式重译 + 定段缓存；llama.cpp 独立进程、仅 127.0.0.1 回环，运行时由你 `brew install llama.cpp`（ADR-0004 不代下可执行文件） | 关闭 |
