...
1. **自定义短语**：静态映射表（`cs`→常用地址等），xtask 校验 + 加密存储；对标 Rime custom_phrase / fcitx5 quickphrase。
2. **Unicode / 符号输入**：`u` 前缀 + 常用符号/序号/货币表；对标 fcitx5 unicode、HeliBoard 符号词典。
3. **简繁转换**：导入 OpenCC 数据（纯数据离线），候选加「繁」切换。
4. **数据备份/恢复**：`userdata.enc` 导出为加密包（导出明文需二次确认），对标 HeliBoard 备份。
