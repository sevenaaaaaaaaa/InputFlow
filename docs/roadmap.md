...
| **M1 可用与记忆** | 用户词加密持久化、剪切板历史、设置面板、macOS 打磨（标点/翻页/中英切换/emoji/符号/简繁）、备份恢复、设计令牌生成器；本地 AI：模型目录/内存推荐/下载与 sha256 校验（`crates/ai` + 设置窗） | 日常可替代系统拼音；钥匙串/加密文件可验证；模型下载可校验、可删除 |
...
- [x] AI 辅助短语：按键序列级短语自动学习（窗口 3 段）+ 前缀补全，不做手动短语表
- [x] Unicode / 符号输入：`u` 前缀，符号 / 序号 / 货币 / 数学 / 希腊 / 箭头，拼音与英文别名检索
- [x] 简繁转换：`crates/zhconv`（OpenCC 词表、最长匹配消歧），菜单「繁体输出」开关
- [x] 端上语音输入（L1）：SFSpeechRecognizer 强制端上识别（不支持即拒绝、**绝不回退云端**）；与打字融合——边说边落字（实时预编辑 + 强调色下划线）、语音结果作候选 #1（空格/1/回车/点击上屏）、打字自动接管（先落已识别文本再拼拼音）；语音词记入用户模型与二元组上下文（`inputflow_record_commit`）
- [x] 同声传译（端上流式翻译）：菜单「同声传译 → 译成英文/日文」武装；识别 final 后原文驻预编辑，候选 [译文, 原文]（空译文标「翻译中…」、失败标「翻译不可用」）；流式断句——定段译文缓存、尾段 0.5s 防抖重译、代次丢弃在途回包；llama.cpp 运行时探测/引导（brew 自装，ADR-0004 不代下可执行文件）+ gemma-3-270m-it-q8 下载与 sha256 校验；llama-server 独立进程 + 仅回环调用，stop/atexit + SIGKILL 兜底防孤儿；`--translate-check` / `--translate-demo` / `--ai-download` CLI 验证
- [x] 语音标点二次确认：普通语音 final 不再自动上屏，候选 [原样, 。, ，, ！, ？]——空格/回车=原样、数字 1-5 选标点、打字吸收（先落原样再拼）、Esc 取消；同声传译模式不走确认（译/原文切换即确认）；`--punct-demo` 快照验证
- [x] 数据备份 / 恢复：带 CRC32 的备份信封 + 密码派生密钥加密包（`.ifbak`），明文导出二次确认
